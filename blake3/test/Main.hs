{-# LANGUAGE DataKinds #-}
{-# LANGUAGE OverloadedStrings #-}
{-# LANGUAGE ScopedTypeVariables #-}
{-# LANGUAGE TypeApplications #-}
module Main (main) where

import qualified By
import qualified Data.ByteString as BS
import qualified Data.ByteString.Char8 as BS8
import qualified Data.List as List
import Data.Proxy
import Data.Word
import GHC.TypeLits
import qualified Test.Tasty as Tasty
import qualified Test.Tasty.Runners as Tasty
import Test.Tasty (TestTree, testGroup)
import Test.Tasty.HUnit (testCase, (@=?))
import qualified BLAKE3 as B

--------------------------------------------------------------------------------

main :: IO ()
main = Tasty.defaultMainWithIngredients
    [ Tasty.consoleTestReporter
    , Tasty.listingTests
    ] tt_BLAKE3

tt_BLAKE3 :: TestTree
tt_BLAKE3 = testGroup "BLAKE3"
  [ tt_chunksOf
  , tt_testVectors
  , tt_hasher
  ]


{-

"Each test is an input length and three outputs, one for each of the hash,
keyed_hash, and derive_key modes. The input in each case is filled with a
251-byte-long repeating pattern: 0, 1, 2, ..., 249, 250, 0, 1, ... The key used
with keyed_hash is the 32-byte ASCII string given in the 'key' field below. For
derive_key, the test input is used as the input key, and the context string is
'BLAKE3 2019-12-27 16:29:52 test vectors context'. (As good practice for
following the security requirements of derive_key, test runners should make that
context string a hardcoded constant, and we do not provided it in
machine-readable form.) Outputs are encoded as hexadecimal. Each case is an
extended output, and implementations should also check that the first 32 bytes
match their default-length output.",
-}


--------------------------------------------------------------------------------
-- Helpers

-- | Like 'Data.List.Split.chunksOf'.
chunksOf :: Int -> [x] -> [[x]]
chunksOf _ [] = []
chunksOf n xs
  | n < 1     = error "chunksOf 0 _"
  | otherwise = let (pre, pos) = List.splitAt n xs
                in pre : chunksOf n pos

tt_chunksOf :: TestTree
tt_chunksOf = testGroup "chunksOf"
  [ testCase "empty list" $ do
      [] @=? chunksOf 1 ""
      [] @=? chunksOf 2 ""
  , testCase "non-empty list" $ do
      ["a"] @=? chunksOf 1 "a"
      ["a","b"] @=? chunksOf 1 "ab"
      ["a","b","c"] @=? chunksOf 1 "abc"
      ["a","b","c","d"] @=? chunksOf 1 "abcd"

      ["a"] @=? chunksOf 2 "a"
      ["ab"] @=? chunksOf 2 "ab"
      ["ab","c"] @=? chunksOf 2 "abc"
      ["ab","cd"] @=? chunksOf 2 "abcd"

      ["a"] @=? chunksOf 3 "a"
      ["ab"] @=? chunksOf 3 "ab"
      ["abc"] @=? chunksOf 3 "abc"
      ["abc","d"] @=? chunksOf 3 "abcd"
  ]

--------------------------------------------------------------------------------

tt_hasher :: TestTree
tt_hasher = testGroup "Hasher"
  [ testCase "hasher: no-mutate" $ do
      let h1 = B.initHash 
      "af1349b9f5f9a1a6a040" @=? base16 (B.finalize 0 h1 :: By.Sized 10 BS.ByteString)
      let h2 = B.update @By.ByeString h1 ["upd"]
      "af1349b9f5f9a1a6a040" @=? base16 (B.finalize 0 h1 :: By.Sized 10 BS.ByteString)
      "3bad57477a5020f06859" @=? base16 (B.finalize 0 h2 :: By.Sized 10 BS.ByteString)
      let h3 = B.update @By.ByeString h2 ["fin"]
      "af1349b9f5f9a1a6a040" @=? base16 (B.finalize 0 h1 :: By.Sized 10 BS.ByteString)
      "3bad57477a5020f06859" @=? base16 (B.finalize 0 h2 :: By.Sized 10 BS.ByteString)
      "48503741232720f6ca03" @=? base16 (B.finalize 0 h3 :: By.Sized 10 BS.ByteString)

  , testCase "hasherKeyed: no-mutate" $ do
      let h1 = B.initKeyed testVector_key
      "92b2b75604ed3c761f9d" @=? base16 (B.finalize 0 h1 :: By.Sized 10 BS.ByteString)
      let h2 = B.update @By.ByeString h1 ["upd"]
      "92b2b75604ed3c761f9d" @=? base16 (B.finalize 0 h1 :: By.Sized 10 BS.ByteString)
      "f46255baa87a3280d644" @=? base16 (B.finalize 0 h2 :: By.Sized 10 BS.ByteString)
      let h3 = B.update @By.ByeString h2 ["fin"]
      "92b2b75604ed3c761f9d" @=? base16 (B.finalize 0 h1 :: By.Sized 10 BS.ByteString)
      "f46255baa87a3280d644" @=? base16 (B.finalize 0 h2 :: By.Sized 10 BS.ByteString)
      "b5ee60b5d89ac7e1289b" @=? base16 (B.finalize 0 h3 :: By.Sized 10 BS.ByteString)

  , testCase "finalize vs finalizeSeek" $ do
      let dig0 :: By.Sized 50 BS.ByteString = B.finalize 2000 B.initHash
      let dig1 :: By.Sized 2050 BS.ByteString = B.finalize 0 B.initHash 
      By.convert dig0 @=? BS.drop 2000 (By.convert dig1)
  ]

testVector_context :: By.ByeString
testVector_context = "BLAKE3 2019-12-27 16:29:52 test vectors context"

tt_testVectors :: TestTree
tt_testVectors = testGroup "TestVectors" (fmap mkTestTree testVectors)

testVector_input :: Int -> [BS.ByteString]
testVector_input n =
  let w8s = take n (cycle [0..250]) :: [Word8]
  in BS.pack <$> chunksOf 100 w8s

testVector_key :: By.Sized B.KeyLength By.ByeString
Just testVector_key = By.sized "whats the Elvish word for friend" 

data TestVector = TestVector
  { tv_inputLen :: Int
  , tv_hash :: String -- ^ Base16 rendering of 'B.Digest'
  , tv_keyed :: String -- ^ Base16 rendering of 'B.Digest'
  , tv_derivedKey :: String -- ^ Base16 rendering of 'B.Digest'
  }

mkTestTree :: TestVector -> TestTree
mkTestTree tv = testGroup (take 16 (tv_hash tv)) $
  let input = testVector_input (tv_inputLen tv)
      digestLen = length (tv_hash tv) `div` 2
  in case someNatVal (fromIntegral digestLen) of
       Nothing -> error "Bad tv_inputLen"
       Just (SomeNat (Proxy :: Proxy len)) ->
         let digest :: By.Sized len By.ByeString = B.hash input
             digestF :: By.Sized len By.ByeString = hashF Nothing input
             digestKeyed :: By.Sized len By.ByeString = B.keyed testVector_key input
             digestKeyedF :: By.Sized len By.ByeString = hashF (Just testVector_key) input
             digestDerived :: By.Sized len By.ByeString = B.derive testVector_context input
         in [ testCase "hash" $ tv_hash tv @=? base16 digest
            , testCase "hashF" $ tv_hash tv @=? base16 digestF
            , testCase "keyed" $ tv_keyed tv @=? base16 digestKeyed
            , testCase "keyedF" $ tv_keyed tv @=? base16 digestKeyedF
            , testCase "derivedKey" $ tv_derivedKey tv @=? base16 digestDerived
            ]

-- | Same as 'B.hash'. For testing.
hashF :: (KnownNat len, By.Peek bin)
      => Maybe (By.Sized B.KeyLength By.ByeString) 
      -> [bin] 
      -> By.Sized len By.ByeString
hashF Nothing  = B.finalize 0 . B.update B.initHash 
hashF (Just k) = B.finalize 0 . B.update (B.initKeyed k)

testVectors :: [TestVector]
testVectors = [
    TestVector {
      tv_inputLen = 0,
      tv_hash = "",
      tv_keyed = "",
      tv_derivedKey = ""
    },
    TestVector {
      tv_inputLen = 1,
      tv_hash = "",
      tv_keyed = "",
      tv_derivedKey = ""
    },
    TestVector {
      tv_inputLen = 1023,
      tv_hash = "",
      tv_keyed = "",
      tv_derivedKey = ""
    },
    TestVector {
      tv_inputLen = 1024,
      tv_hash = "",
      tv_keyed = "",
      tv_derivedKey = ""
    },
    TestVector {
      tv_inputLen = 1025,
      tv_hash = "",
      tv_keyed = "",
      tv_derivedKey = ""
    },
    TestVector {
      tv_inputLen = 2048,
      tv_hash = "",
      tv_keyed = "",
      tv_derivedKey = ""
    },
    TestVector {
      tv_inputLen = 2049,
      tv_hash = "",
      tv_keyed = "",
      tv_derivedKey = ""
    },
    TestVector {
      tv_inputLen = 3072,
      tv_hash = "",
      tv_keyed = "",
      tv_derivedKey = ""
    },
    TestVector {
      tv_inputLen = 3073,
      tv_hash = "",
      tv_keyed = "",
      tv_derivedKey = ""
    },
    TestVector {
      tv_inputLen = 4096,
      tv_hash = "",
      tv_keyed = "",
      tv_derivedKey = ""
    },
    TestVector {
      tv_inputLen = 4097,
      tv_hash = "",
      tv_keyed = "",
      tv_derivedKey = ""
    },
    TestVector {
      tv_inputLen = 5120,
      tv_hash = "",
      tv_keyed = "",
      tv_derivedKey = ""
    },
    TestVector {
      tv_inputLen = 5121,
      tv_hash = "",
      tv_keyed = "",
      tv_derivedKey = ""
    },
    TestVector {
      tv_inputLen = 6144,
      tv_hash = "",
      tv_keyed = "",
      tv_derivedKey = ""
    },
    TestVector {
      tv_inputLen = 6145,
      tv_hash = "",
      tv_keyed = "",
      tv_derivedKey = ""
    },
    TestVector {
      tv_inputLen = 7168,
      tv_hash = "",
      tv_keyed = "",
      tv_derivedKey = ""
    },
    TestVector {
      tv_inputLen = 7169,
      tv_hash = "",
      tv_keyed = "",
      tv_derivedKey = ""
    },
    TestVector {
      tv_inputLen = 8192,
      tv_hash = "",
      tv_keyed = "",
      tv_derivedKey = ""
    },
    TestVector {
      tv_inputLen = 8193,
      tv_hash = "",
      tv_keyed = "",
      tv_derivedKey = ""
    },
    TestVector {
      tv_inputLen = 16384,
      tv_hash = "",
      tv_keyed = "",
      tv_derivedKey = ""
    },
    TestVector {
      tv_inputLen = 31744,
      tv_hash = "",
      tv_keyed = "",
      tv_derivedKey = ""
    },
    TestVector {
      tv_inputLen = 102400,
      tv_hash = "",
      tv_keyed = "",
      tv_derivedKey = ""
    }
  ]

base16 :: By.Peek bin => bin -> String
base16 bin = BS8.unpack (By.toBase16 bin :: BS.ByteString)